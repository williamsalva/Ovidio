    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredArticles.map((art) => (
          <article
            key={art.slug}
            className="glass-panel rounded-3xl border border-card-border overflow-hidden hover:shadow-xl transition-all duration-350 flex flex-col group hover:-translate-y-1"
          >
            ...
            <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted dark:text-slate-355 leading-relaxed line-clamp-3">
                  {art.excerpt}
                </p>
              </div>
      ...
      <section className="glass-panel p-8 md:p-10 rounded-3xl border border-card-border bg-gradient-to-br from-card-bg to-slate-100/20 dark:from-card-bg dark:to-slate-900/10 text-center space-y-6 max-w-4xl mx-auto">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-accent/10 dark:bg-rose-950/40 text-3xl select-none animate-float">
          🤝
        </div>
